...
AudioBufferList *MyTPCircularBufferNextBufferList(TPCircularBuffer *buffer, AudioTimeStamp *outTimestamp) {
...
void MyTPCircularBufferConsumeNextBufferList(TPCircularBuffer *buffer) {
    int32_t dontcare;
    TPCircularBufferABLBlockHeader *block = TPCircularBufferTail(buffer, &dontcare);
    if ( !block ) return;
